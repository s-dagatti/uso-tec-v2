    ...
    if not df_filtrado_aptas.empty:
        ...
        with tab_guiado:
            ...
            if cols_presentes:
                ...
                if not df_ga.empty:
                    # 2. Filtrar: mantener solo valores estrictamente mayores a 0.1% (ignora 0, nulos y valores <= 0.1)
                    cols_clean = []
                    for col in cols_presentes:
                        clean_col = f"{col}_clean"
                        cols_clean.append(clean_col)
                        df_ga[col] = pd.to_numeric(df_ga[col], errors="coerce")
                        df_ga[clean_col] = df_ga[col].where(df_ga[col] > 0.1)
                    ...
                    # Promedio individual de cada tecnología (> 0.1%), si no hay datos aporta 0.0 para dividir siempre entre 4
                    ...
                    # B. Cantidad de Máquinas Activas en el Período (registros > 0.1%)
                    ...
                    with kpi_gen2:
                        st.metric(
                            label="Máquinas Activas (Uso > 0.1% en el período)",
                    ...
                    # ------------------------------------------------------------------
                    # BLOQUE 3: TABLA DETALLE POR MÁQUINA (SOLO MÁQUINAS CON USO > 0.1%)
                    # ------------------------------------------------------------------
                    st.markdown("---")
                    ...
                    if group_keys and "AutoPath™ Activo" in df_ga.columns:
                        ...
                        # FILTRO: Excluir máquinas cuyo promedio sea menor o igual a 0.1% o nulo
                        ap_periodo = ap_periodo[(ap_periodo["AutoPath™ Activo"].notna()) & (ap_periodo["AutoPath™ Activo"] > 0.1)].copy()
                        ...
                        if not ap_periodo.empty:
                            ...
                            st.info("ℹ️ No se encontraron máquinas con uso registrado (> 0.1%) de AutoPath™ en el período seleccionado.")
