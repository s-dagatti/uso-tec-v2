    ...
    if not df_filtrado_aptas.empty:
        ...
        with tab_guiado:
            ...
            if cols_presentes:
                ...
                if not df_ga.empty:
                    # 2. Filtrar: mantener solo valores estrictamente mayores a 0.5% (ignora 0, nulos y valores <= 0.5)
                    cols_clean = []
                    for col in cols_presentes:
                        clean_col = f"{col}_clean"
                        cols_clean.append(clean_col)
                        df_ga[col] = pd.to_numeric(df_ga[col], errors="coerce")
                        df_ga[clean_col] = df_ga[col].where(df_ga[col] > 0.5)
                    ...
                    # Promedio individual de cada tecnología (> 0.5%), si no hay datos aporta 0.0 para dividir siempre entre 4
                    ...
                    # B. Cantidad de Máquinas Activas en el Período (registros > 0.5%)
                    ...
                    with kpi_gen2:
                        st.metric(
                            label="Máquinas Activas (Uso > 0.5% en el período)",
                    ...
                    # ------------------------------------------------------------------
                    # BLOQUE 3: TABLA DETALLE POR MÁQUINA (SOLO MÁQUINAS CON USO > 0.5%)
                    # ------------------------------------------------------------------
                    st.markdown("---")
                    ...
                    if group_keys and "AutoPath™ Activo" in df_ga.columns:
                        ...
                        # FILTRO: Excluir máquinas cuyo promedio sea menor o igual a 0.5% o nulo
                        ap_periodo = ap_periodo[(ap_periodo["AutoPath™ Activo"].notna()) & (ap_periodo["AutoPath™ Activo"] > 0.5)].copy()
                        ...
                        if not ap_periodo.empty:
                            ...
                            st.info("ℹ️ No se encontraron máquinas con uso registrado (> 0.5%) de AutoPath™ en el período seleccionado.")
