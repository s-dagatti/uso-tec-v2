...
def guardar_en_github(df, repo, path, token, commit_msg="Actualización de serie histórica"):
    ...
    if res_put.status_code in [200, 201]:
        return True, "Base de datos histórica actualizada con éxito en GitHub."
    else:
...
st.subheader("📦 Estado Actual de la Base Histórica")
if df_historico is not None and not df_historico.empty:
    ...
    if 'Estado Licencia' in df_historico.columns:
        col_h3.metric("🟢 Licencias Vigentes", (df_historico['Estado Licencia'] == 'Vigente').sum())
        col_h4.metric("🔴 Licencias Vencidas", (df_historico['Estado Licencia'] == 'Vencida').sum())
    ...
    try:
        ...
        if uploaded_file_maquinas is not None:
            ...
            df_monitores = df_monitores[[
                'Número de serie de emparejamiento', 'Número de serie', 'Modelo', 'Versión de software'    
            ]].rename(columns={
        ...
        if uploaded_file_activaciones is not None and 'Hardware Monitor' in df_nuevo.columns:
            ...
            df_licencias_gen4 = df_gen4_unique[[
                ...
                'Producto': 'Licencia', 'Duración': 'Duración Licencia',
                'Fecha de inicio': 'Comienzo Licencia', 'Fecha final': 'Fin Licencia'
            })
        ...
        if uploaded_file_gen5 is not None:
            ...
            for col, field_map in [
                ('Licencia', map_licencia), ('Duración Licencia', map_duracion),
                ('Comienzo Licencia', map_comienzo), ('Fin Licencia', map_fin)
            ]:
        ...
        # --- G. CONCATENAR CON LA BASE HISTÓRICA ---
        if df_historico is not None and not df_historico.empty:
            df_final = pd.concat([df_historico, df_nuevo], ignore_index=True)
            df_final = df_final.drop_duplicates()  # Evita duplicar exactamente el mismo reporte si se resube
        else:
            df_final = df_nuevo.copy()

        # --- H. MOSTRAR RESULTADOS Y NUEVA SERIE HISTÓRICA ---
        st.success("✨ Nuevos datos procesados y acoplados exitosamente a la serie histórica.")
        
        st.subheader("📊 Métrica del Nuevo Consolidado")
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Filas (Serie Histórica)", len(df_final), delta=f"+{len(df_nuevo)} nuevos")
        c2.metric("Nueva Fecha de Actualización", obtener_ultima_fecha(df_final))
        c3.metric("🟢 Licencias Vigentes", (df_final['Estado Licencia'] == 'Vigente').sum())
        c4.metric("🔴 Licencias Vencidas", (df_final['Estado Licencia'] == 'Vencida').sum())
        ...
        with col_btn1:
            if st.button("📌 Actualizar Base Histórica en GitHub", type="primary"):
                try:
                    with st.spinner("Sincronizando con GitHub..."):
                        exito, mensaje = guardar_en_github(df_final, gh_repo, gh_path, gh_token)
                        if exito:
                            st.success(f"✅ {mensaje}")
                            st.cache_data.clear()  # Limpia la caché de Streamlit para recargar la nueva versión
                        else:
                            st.error(f"❌ {mensaje}")
