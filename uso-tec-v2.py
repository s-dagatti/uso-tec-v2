...
st.set_page_config(page_title="Carga y Consolidación de Datos - Conci", layout="wide")

st.title("📂 Tablero de Carga y Consolidación de Datos")
st.markdown("Sube los archivos para procesar, limpiar, consolidar y guardar la base en GitHub.")
...
def guardar_en_github(df, repo, path, token, commit_msg="Actualización de base de datos consolidada"):
    """
    Sube o actualiza el DataFrame como CSV en el repositorio de GitHub indicado.
    """
    ...
    # Convertir DataFrame a CSV encodeado en base64
    csv_bytes = df.to_csv(index=False).encode('utf-8')
    content_b64 = base64.b64encode(csv_bytes).decode('utf-8')
    
    # Verificar si el archivo existe para obtener su SHA (requerido para actualizar)
...
if uploaded_file_analizador is not None:
    try:
        ...
        if uploaded_file_orgs is not None:
            ...
            df_final = pd.merge(df_final, df_orgs_unique, left_on='Identificador de organización', right_on='Org ID', how='left')
        ...
        # 3. EMPAREJAMIENTOS (MÁQUINAS / MONITORES)
        if uploaded_file_maquinas is not None:
            ...
            df_final = pd.merge(df_final, df_monitores, left_on='Número de serie de la máquina', right_on='Número de serie de emparejamiento', how='left')
        ...
        if uploaded_file_activaciones is not None and 'Hardware Monitor' in df_final.columns:
            ...
            df_final = pd.merge(df_final, df_licencias_gen4, left_on='Hardware Monitor', right_on='Tornillería', how='left')
        ...
        col3.metric("🟢 Vigentes", (df_final['Estado Licencia'] == 'Vigente').sum())
        col4.metric("🔴 Vencidas", (df_final['Estado Licencia'] == 'Vencida').sum())
        col5.metric("⚪ No tiene", (df_final['Estado Licencia'] == 'No tiene').sum())

        st.subheader("Vista previa de la base consolidada")
        st.dataframe(df_final, use_container_width=True)
        
        # --- ACCIONES: GUARDAR EN GITHUB / DESCARGAR ---
        st.markdown("---")
        st.subheader("📌 Guardar y Exportar Base Consolidada")
        
        col_gh, col_dl = st.columns([1, 1])
        
        with col_gh:
            if st.button("🚀 Actualizar Base de Datos en GitHub", type="primary"):
                try:
